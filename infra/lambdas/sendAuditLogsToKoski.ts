...
const QUEUE_URL = process.env.QUEUE_URL! // cross-account queue URL
const SQS_REGION = process.env.SQS_REGION || process.env.AWS_REGION

const sqs = new SQSClient({ region: SQS_REGION })
...
export const handler = async (event: CloudWatchLogsEvent) => {
  ...
  for (const e of decoded.logEvents) {
    ...
    const cmdInput: any = { QueueUrl: QUEUE_URL, MessageBody: body }
