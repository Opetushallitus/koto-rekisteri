...
class KoealustaMappingService(
    ...
    fun mapToExceptionsAndSuoritukset(
        suorituksetResponse: KoealustaSuorituksetResponse,
    ): List<Pair<OppijanumeroException?, KielitestiSuoritus>> =
        suorituksetResponse.users.flatMap { user ->
            val (ex, oppijanumero) = getOppijanumero(user)
            user.completions.map { completion ->
                Pair(
                    ex,
                    completionToEntity(
                        user,
                        oppijanumero.toString(),
                        completion,
                    ),
                )
            }
    ...
    fun getOppijanumero(user: User) = oppijanumeroService.getOppijanumeroOrError(user.toOppija())
