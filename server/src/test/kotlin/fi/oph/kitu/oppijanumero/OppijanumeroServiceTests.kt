...
class OppijanumeroServiceTests {
    fun addCasAuthenticatedService(
        ...
        // Expectation 5: Request *with* JSESSIONID cookie
        // No further requests expected: HTTP POST http://localhost:8080/oppijanumero-service/yleistunniste/hae
        // 2 request(s) executed:
        // POST http://localhost:8080/oppijanumero-service/yleistunniste/hae, headers: [Content-Type:"application/json"]
        // GET http://localhost:8080/oppijanumero-service/j_spring_cas_security_check?ticket=65432-1

        mockServer
            .expect(requestTo(expectedUri))
            // .andExpect(header("Cookie", Matchers.containsString("JSESSIONID=")))
            .andRespond(
